...
- `scripts/lease-paths.ps1`(`-LockDir` パラメータ追加のみ。`../escape` 等が正規化で
  潰れる場合のみ、owned_paths の path traversal 拒否を同ファイル内で最小修正可)
- `tests/lease-paths.Tests.ps1`(create)
- `tests/delegate-codex.Tests.ps1`(create)
- `.github/workflows/ci.yml`(create)
- `plans/README.md`(本プランの status 行のみ)
- `plans/001-ci-pester-baseline.md`(Done criteria チェック反映のみ)

**Out of scope** (do NOT touch, even though they look related):
- `scripts/delegate-codex.ps1` の動作変更(105–111 行の冗長 if/else 含む — Plan 002 で扱う)
- `.agents/` 配下のドキュメント・スキル(review packet 保存は任意)
