...
PowerShell 側は出力を `-Raw` 相当で受けて `"`u{0}"` で split する
(`& git ... | Out-String` は改行を足すので、`$out -join "`n"` 経由ではなく
配列 join → split の既存 Invoke-GitChecked を `-z` 対応に拡張: NUL を含む生文字列を
返す `-RawOutput` スイッチを足すのが最小)。
...
- README の「既知の制限」記述(先頭スペースファイル名)を 1 行修正(EN/JA)
